---
source: neso
dataset_key: regional_intensity_pt24h_regionid
vendor: National Energy System Operator (NESO)
last_verified: 2026-05-08
layer_coverage: bronze, silver
---

# NESO - Region-id carbon intensity previous 24h (`regional_intensity_pt24h_regionid`)
...
curl --ssl-no-revoke -X GET \
  "https://api.carbonintensity.org.uk/regional/intensity/2024-01-15T00:00Z/pt24h/regionid/13" \
...
**Path pattern**: `{data_root}/bronze/neso/regional_intensity_pt24h_regionid/<year>/<month>/<day>/raw_<timestamp>_<hash>.json`
...
its own `data[]` of period rows. Each period row holds `intensity` and
`generationmix`.

```json
{"data":{"regionid":13,"dnoregion":"UKPN London","shortname":"London","data":[{"from":"2026-05-04T23:30Z","to":"2026-05-05T00:00Z","intensity":{"forecast":210,"index":"high"},"generationmix":[{"fuel":"gas","perc":42.0},{"fuel":"imports","perc":24.0}]}]}}
```
...
**Path pattern**: `{data_root}/silver/neso/regional_intensity_pt24h_regionid/year=<YYYY>/month=<MM>/regional_intensity_pt24h_regionid_<YYYYMMDD>.parquet`
**Transformer class**: `gridflow.silver.neso.carbon_intensity.RegionalIntensityPt24HRegionidTransformer`
