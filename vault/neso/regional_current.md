---
source: neso
dataset_key: regional_current
vendor: National Energy System Operator (NESO)
last_verified: 2026-05-09
layer_coverage: bronze, silver
---

# NESO - Current regional carbon intensity (`regional_current`)
...
| Publication lag | Forecast ahead; regional actual field usually absent in docs examples |
...
**Path pattern**: `{data_root}/bronze/neso/regional_current/<year>/<month>/<day>/raw_<timestamp>_<hash>.json`
...
region**, not at the period level. There is no `postcode` field on regions in
this shape (postcodes only appear on `regional_postcode`-style routes).
...
**Path pattern**: `{data_root}/silver/neso/regional_current/year=<YYYY>/month=<MM>/regional_current_<YYYYMMDD>.parquet`
**Transformer class**: `gridflow.silver.neso.carbon_intensity.RegionalCurrentTransformer`
...
- **Period-keyed shape vs silver row builder — RESOLVED in V2 (2026-05-09).** `silver/neso/carbon_intensity.py::_rows_from_region_period` now reads `intensity` and `generationmix` from `region` first, falling back to `period`. Region-keyed routes continue to work unchanged. See gridflow commit `fix(V2-B):` on `claude/lucid-mccarthy-9ed3e0`. Live re-validation 2026-05-09: 7938 silver rows from `/regional/intensity/2026-05-09T00:00Z/fw24h`, 0% null on `forecast_gco2_kwh`, `fuel`, `generation_percentage`.

---

## Changelog

- **2026-05-09 — V2-FIX-02.** Silver `_rows_from_region_period` now reads `intensity`/`generationmix` from whichever level (region or period) holds the data. Affects this dataset and 4 siblings (`regional_intensity`, `regional_intensity_fw24h`, `regional_intensity_fw48h`, `regional_intensity_pt24h`). Regression fixture-backed tests added under `tests/unit/test_neso_regional_silver.py`. Out-of-scope follow-up: existing silver tables for these 5 datasets contain null carbon/mix and require re-running silver from existing bronze.
