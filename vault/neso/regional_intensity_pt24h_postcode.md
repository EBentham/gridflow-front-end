---
source: neso
dataset_key: regional_intensity_pt24h_postcode
vendor: National Energy System Operator (NESO)
last_verified: 2026-05-08
layer_coverage: bronze, silver
---

# NESO - Postcode regional carbon intensity previous 24h (`regional_intensity_pt24h_postcode`)
...
curl --ssl-no-revoke -X GET \
  "https://api.carbonintensity.org.uk/regional/intensity/2024-01-15T00:00Z/pt24h/postcode/RG10" \
...
**Path pattern**: `{data_root}/bronze/neso/regional_intensity_pt24h_postcode/<year>/<month>/<day>/raw_<timestamp>_<hash>.json`
...
its own `data[]` of period rows. The region object includes a `postcode` field
echoed from the request. Each period row holds `intensity` and `generationmix`.

```json
{"data":{"regionid":12,"dnoregion":"SSE South","shortname":"South England","postcode":"RG41","data":[{"from":"2026-05-04T23:30Z","to":"2026-05-05T00:00Z","intensity":{"forecast":268,"index":"very high"},"generationmix":[{"fuel":"gas","perc":62.5},{"fuel":"imports","perc":33.4}]}]}}
```
...
**Path pattern**: `{data_root}/silver/neso/regional_intensity_pt24h_postcode/year=<YYYY>/month=<MM>/regional_intensity_pt24h_postcode_<YYYYMMDD>.parquet`
**Transformer class**: `gridflow.silver.neso.carbon_intensity.RegionalIntensityPt24HPostcodeTransformer`
