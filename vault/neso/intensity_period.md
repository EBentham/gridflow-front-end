---
source: neso
dataset_key: intensity_period
vendor: National Energy System Operator (NESO)
last_verified: 2026-05-08
layer_coverage: bronze, silver
---

# NESO - Settlement-period national carbon intensity (`intensity_period`)
...
| Publication lag | Forecast ahead, actual as estimated after the selected period |
...
|-----------|------|----------|-------------|---------|
| date | string | Yes | Path parameter: Date in YYYY-MM-DD format | 2024-01-15 |
| period | string | Yes | Path parameter: Half-hour settlement period between 1 and 48 in official docs | 42 |

### Working curl example

```bash
curl --ssl-no-revoke -X GET \
  "https://api.carbonintensity.org.uk/intensity/date/2024-01-15/42" \
...
**Path pattern**: `{data_root}/bronze/neso/intensity_period/<year>/<month>/<day>/raw_<timestamp>_<hash>.json`
...
**Path pattern**: `{data_root}/silver/neso/intensity_period/year=<YYYY>/month=<MM>/intensity_period_<YYYYMMDD>.parquet`
**Transformer class**: `gridflow.silver.neso.carbon_intensity.IntensityPeriodTransformer`
...
- **`period`**: official docs say 1-48; connector iterates GB settlement period counts, including 46/50 period DST days, to match local settlement reality.
