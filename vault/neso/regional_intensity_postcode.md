---
source: neso
dataset_key: regional_intensity_postcode
vendor: National Energy System Operator (NESO)
last_verified: 2026-05-08
layer_coverage: bronze, silver
---

# NESO - Postcode regional carbon intensity range (`regional_intensity_postcode`)
...
curl --ssl-no-revoke -X GET \
  "https://api.carbonintensity.org.uk/regional/intensity/2024-01-15T00:00Z/2024-01-16T00:00Z/postcode/RG10" \
...
**Path pattern**: `{data_root}/bronze/neso/regional_intensity_postcode/<year>/<month>/<day>/raw_<timestamp>_<hash>.json`
...
its own `data[]` of period rows over the requested range. The region object
includes a `postcode` field echoed from the request. Each period row holds
`intensity` and `generationmix`.

```json
{"data":{"regionid":12,"dnoregion":"SSE South","shortname":"South England","postcode":"RG41","data":[{"from":"2026-05-06T00:00Z","to":"2026-05-06T00:30Z","intensity":{"forecast":210,"index":"high"},"generationmix":[{"fuel":"gas","perc":42.0},{"fuel":"imports","perc":22.0}]}]}}
```
...
**Path pattern**: `{data_root}/silver/neso/regional_intensity_postcode/year=<YYYY>/month=<MM>/regional_intensity_postcode_<YYYYMMDD>.parquet`
**Transformer class**: `gridflow.silver.neso.carbon_intensity.RegionalIntensityPostcodeTransformer`
